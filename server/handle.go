...
// 			ok, err := sess.TryAcquireLock(SeatID,filePath, lockMode)
// 			// ok, err := sess.TryAcquireLock(SeatID,filePath, lockMode)
// 			//  api.FilePath(req.ClientID), req.Mode
// 			if err != nil {
...
func (h *Handler) TryAcquireLock(req api.TryAcquireLockRequest, res *api.TryAcquireLockResponse) error {
	...
	isSuccessful, err := sess.TryAcquireLock(req.SeatID, api.FilePath(req.ClientID), req.Mode)
	if err != nil {
		return err
	}
